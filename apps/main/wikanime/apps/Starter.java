package wikanime.apps;

import org.springframework.boot.SpringApplication;
import org.springframework.boot.WebApplicationType;
import org.springframework.context.ConfigurableApplicationContext;
import wikanime.apps.backoffice.backend.BackofficeBackendApplication;
import wikanime.apps.backoffice.frontend.BackofficeFrontendApplication;
import wikanime.apps.mwo.backend.MwoBackendApplication;
...
public final class Starter {
    ...
    private static HashMap<String, Class<?>> applications() {
        ...
        applications.put("mooc_backend", MwoBackendApplication.class);
    ...
    private static HashMap<String, HashMap<String, Class<?>>> commands() {
        ...
        commands.put("mooc_backend", MwoBackendApplication.commands());
    ...
    private static void ensureApplicationExist(String applicationName) {
        if (!applications().containsKey(applicationName)) {
            throw new RuntimeException(String.format("The application <%s> doesn't exist. Valid:\n- %s", applicationName, String.join("\n- ", applications().keySet())));
        }
    }
    ...
    private static void ensureCommandExist(String applicationName, String commandName) {
        if (!"server".equals(commandName) && !existCommand(applicationName, commandName)) {
            throw new RuntimeException(String.format("The command <%s> for application <%s> doesn't exist. Valids (application.command):\n- api\n- %s", commandName, applicationName, String.join("\n- ", commands().get(applicationName).keySet())));
        }
    }
