package wikanime.shared.infrastructure.bus.event.rabbitmq;
...
import wikanime.shared.domain.Utils;
import wikanime.shared.domain.bus.event.DomainEvent;
import wikanime.shared.infrastructure.bus.event.DomainEventJsonDeserializer;
import wikanime.shared.infrastructure.bus.event.DomainEventSubscribersInformation;
...
public final class RabbitMqDomainEventsConsumer {
    ...
    public RabbitMqDomainEventsConsumer(RabbitListenerEndpointRegistry registry, DomainEventSubscribersInformation information, DomainEventJsonDeserializer deserializer, ApplicationContext context, RabbitMqPublisher publisher) {
    ...
    public void consumer(Message message) throws Exception {
        ...
        Object subscriber = domainEventSubscribers.containsKey(queue) ? domainEventSubscribers.get(queue) : subscriberFor(queue);
        ...
        try {
            ...
            throw new Exception(String.format("The subscriber <%s> should implement a method `on` listening the domain event <%s>", queue, domainEvent.eventName()));
    ...
    private void sendMessageTo(String exchange, Message message, String queue) {
        ...
        MessageBuilder.fromMessage(message).andProperties(MessagePropertiesBuilder.newInstance().setContentEncoding("utf-8").setContentType("application/json").copyHeaders(headers).build());
